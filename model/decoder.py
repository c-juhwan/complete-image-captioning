...
class Decoder(nn.Module):
    ...
    def forward(self, features, caption_ids, lengths):
        ...
        if self.decoder_type == 'transformer':
            trg_mask = self.generate_square_subsequent_mask(decoder_input.size(0), device=decoder_input.device) # (max_seq_len, max_seq_len)
            trg_key_padding_mask = (caption_ids == 0) # (batch_size, max_seq_len)

            decoder_input = decoder_input + self.pos_encoder(decoder_input) # (batch_size, max_seq_len, embed_dim)
            decoder_input = decoder_input.permute(1, 0, 2) # (max_seq_len, batch_size, embed_dim)
            decoder_output = self.decoder(decoder_input, features, 
                                          tgt_mask=trg_mask, tgt_key_padding_mask=trg_key_padding_mask) # (max_seq_len, batch_size, embed_dim)      
        else: # rnn, lstm, gru
            ...
            decoder_output = self.rnn_output_to_embed_linear(decoder_output) # (max_seq_len, batch_size, embed_dim)
        
        decoder_output = decoder_output.permute(1, 0, 2) # (batch_size, max_seq_len, embed_dim)
